...
import static org.opennars.lab.grid2d.main.Cell.Logic.NOT;
import static org.opennars.lab.grid2d.main.Cell.Logic.OFFSWITCH;
import static org.opennars.lab.grid2d.main.Cell.Logic.OR;
import static org.opennars.lab.grid2d.main.Cell.Logic.SWITCH;
import static org.opennars.lab.grid2d.main.Cell.Logic.UNCERTAINBRIDGE;
import static org.opennars.lab.grid2d.main.Cell.Logic.WIRE;
import static org.opennars.lab.grid2d.main.Cell.Logic.XOR;
        ...
        if((r.machine==Machine.Light || r.machine==Machine.Turret) && r.charge==1)
        {
            if(r.light!=1.0f) {
                ...
                if(r.machine==Machine.Turret) {
                    ...
                    if(!nope) {
                        TestChamber.space.add(new Pizza(i, j, ""));
        ...
        if(selected.material==Material.Wirecutters || selected.material==Material.Pizza || selected.material==Material.Door || selected.logic==Logic.OFFSWITCH || selected.logic==Logic.SWITCH || selected.machine==Machine.Light || selected.machine==Machine.Turret) //or other entity...
        {
            ...
            if(readCells[x][y].name.equals("")) {
                ...
                if(selected.logic==Logic.OFFSWITCH) {
                    ...
                    if(TestChamber.curiousity) {
                        space.nar.addInput("<(^go-to,{SELF}," + readCells[x][y].name + ") =/> <{SELF} --> [curious]>>.");
                        space.nar.addInput("<(^activate,{SELF}," + readCells[x][y].name + ") =/> <{SELF} --> [curious]>>.");
                        space.nar.addInput("<(^deactivate,{SELF}," + readCells[x][y].name + ") =/> <{SELF} --> [curious]>>.");
                    }
                }
